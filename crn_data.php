	...
	$ResponseXML .= "";
		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
							...
							if ($_GET["mstatus"]=="invno"){
								...
								//echo $a;
                                                                if($stname == ""){
                                                                    $sql = mysql_query("SELECT * FROM cred where type = '0' and C_REFNO like  '$letters%'  order by C_DATE desc limit 50") or die(mysql_error());
                                                                }else{
                                                                    $sql = mysql_query("SELECT * FROM cred where type = '1' and C_REFNO like  '$letters%'  order by C_DATE desc limit 50") or die(mysql_error());
                                                                }
  ...
  echo $ResponseXML;
	...
	if($_GET["Command"]=="pass_crnno")
	{
		...
		if ($row = mysql_fetch_array($result)){
			...
			$ResponseXML .= "<ch_date><![CDATA[".$row["ch_date"]."]]></ch_date>";
			$ResponseXML .= "<ch_branch><![CDATA[".$row["ch_branch"]."]]></ch_branch>";
		} else {
			...
			$ResponseXML .= "<ch_date><![CDATA[]]></ch_date>";
			$ResponseXML .= "<ch_branch><![CDATA[]]></ch_branch>";
		}
	...
	/*		$_SESSION["CURRENT_DOC"] = 1;      //document ID
		...
		mysql_query("START TRANSACTION", $dbinv);	 	
			...
			$sql="Insert into c_bal (REFNO, SDATE, trn_type, CUSCODE, AMOUNT, BALANCE, brand, DEP, SAL_EX, vatrate, RNO, flag1, DEV, CANCELL, old, active, totpay, svatref, company, VAT, btt, VAT_VAL, SVAT, btt_rate) values 
			('".$_GET["crnno"]."', '".$_GET["crndate"]."', 'CNT', '".$_GET["cus_code"]."', '".$mgrand_tot."', '".$mgrand_tot."', '".$_GET["brand"]."', '".$_GET["department"]."', '".$_GET["salesrep"]."', '".$row["vatrate"]."', '".$_GET["txtrno"]."', '".$mcash."', '".$_SESSION['dev']."', '0', '0', 1, 0, '".$row_crnfrm["svatref"]."', '".$_SESSION['company']."', '$vatType', $nbt, $mtot1, $svatValue, '".$row['nbt']."')";
			$result=mysql_query($sql, $dbinv);
			if ($result!=1){ $sql_status=1; }
                        ...
                        $sqlLedger = "Insert into ledger(l_refno, l_date, l_code, l_amount, l_flag, l_flag1, acyear, ComCode, c_remarks) Values ('" . $_GET["crnno"] . "', '" . $ldate . "', '" . $_GET["accno2"] . "', " . $orgAmount . ", 'CNT', 'DEB', '$ayear', '" . $_SESSION['company'] . "', '".$_GET["cus_code"]."')";
                        ...
                        $sqlGlPost = "select * from gl_posting where docname = 'CREDIT NOTE' and action = 'ADD_DEBTORS'";    
                        $result=mysql_query($sqlGlPost, $dbinv);
                        $rowGlPost = mysql_fetch_array($result);

                        $sqlLedger = "Insert into ledger(l_refno, l_date, l_code, l_amount, l_flag, l_flag1, acyear, ComCode, c_remarks) Values ('" . $_GET["crnno"] . "', '" . $ldate . "', '" . $rowGlPost["l_code"] . "', " . $amount . ", 'CNT', '" . $rowGlPost['entry_flag'] . "', '$ayear', '" . $_SESSION['company'] . "', '".$_GET["cus_code"]."')";
                        $result1=mysql_query($sqlLedger, $dbinv);
                        if ($result1!=1){ $sql_status=223; }  
	  ...
	  ('".$_GET["crnno"]."', '".$_GET["crndate"]."', '".$_GET["crnno"]."', ".$_GET["amount"].", ".$_GET["amount"].", ".(-1*$_GET["amount"]).", '".$_GET["cus_code"]."', '".$_GET["cus_name"]."', '".$_GET["salesrep"]."',  '".$_SESSION['dev']."', 0, 0, 0, '0', '".$_GET["department"]."', 'CNT', 'OVER')";
			$result1=mysql_query($sql1, $dbinv);
			if ($result1!=1){ $sql_status=1; }	
					
			$sql="Insert into cred (C_REFNO, C_DATE, C_INVNO, C_CODE, C_PAYMENT, C_REMARK, C_SALEX, Brand, DEV, CANCELL, company) values 
			('".$_GET["crnno"]."', '".$_GET["crndate"]."', '".$_GET["invno"]."', '".$_GET["cus_code"]."', '".$_GET["amount"]."', '".$remarks."', '".$_GET["salesrep"]."', '".$_GET["brand"]."', '".$_SESSION['dev']."', '0', '".$_SESSION['COMCODE']."') ";
			//echo $sql;
...
//& " values('" & Trim(txtrefno) & "','" & DateValue(dtdate) & "','" & Trim(txt_invno) & "', '" & Trim(txt_cuscode) & "'," & Val(Format(txtamount, General)) & ",'" & Trim(txt_remark) & "','" & Trim(Left(Com_rep, 5)) & "','" & Trim(cmbbrand) & "','0')"
			...
			$sql="update invpara set CRN = CRN + 1 where COMCODE = '" .$_SESSION['COMCODE']. "'";
			$result=mysql_query($sql, $dbinv);
			if ($result!=1){ $sql_status=1; }	
