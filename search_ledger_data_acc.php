...
//echo $res;
		...
		$ResponseXML .= "<table width=\"735\" border=\"0\" class=\"form-matrix-table\">
							...
							while($row = mysql_fetch_array($result)){
								...
								$gridNo = $_GET["gridNo"];
							$ResponseXML .= "<tr>
                              <td onclick=\"ledgno('$cuscode', '$stname', '$gridNo');\">".$row['c_code']."</a></td>
                              <td onclick=\"ledgno('$cuscode', '$stname', '$gridNo');\">".$row['c_name']."</a></td>
...
if ($_GET["Command"]=="search_custom_bank"){
...
if ($_GET["Command"]=="pass_cash_rec_grid"){
	...
	if($row = mysql_fetch_array($result)){							
		$ResponseXML .= "<c_code><![CDATA[".$row['c_code']."]]></c_code>";
		$ResponseXML .= "<c_name><![CDATA[".$row['c_name']."]]></c_name>";
		$ResponseXML .= "<gridNo><![CDATA[".$_GET["gridNo"]."]]></gridNo>";
	}
...
if ($_GET["Command"]=="pass_cus_cash_rec")
{
	...
	echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
			...
			if ($_GET['refno']==""){
				...
				$sql = mysql_query("Select C_CODE, REF_NO, deli_date, GRAND_TOT, TOTPAY, SDATE from s_salma where C_CODE='".$_GET['custno']."' and GRAND_TOT > TOTPAY and CANCELL='0' and SAL_EX='".$_GET['refno']."'  ORDER BY SDATE") or die(mysql_error());
			}	
				while($row = mysql_fetch_array($sql)){
