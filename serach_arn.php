      ...
      <td width="603"  background="" ><input type="text" size="29" name="invdate" id="invdate" value="" class="txtbox"/></td>
                ...
                <div class="CSSTableGenerator" id="filt_table">  <table width="735" border="0" class=\"form-matrix-table\">
						...
						if ($_GET["mstatus"]!="gin"){
                                                        $phrase = "";
                                                        if($stname == "arn_1"){
                                                            $phrase = "s_purmas where CANCEL!='1'";
                                                        }else if($stname == "arn_1_ord"){
                                                            $phrase = "s_ordmas";
                                                        }else{
                                                            $phrase = "s_purmas_tmp where CANCEL!='1'";
                                                        }
...
//							} else {
								$sql="SELECT * FROM $phrase order by SDATE desc ";
//							}
							echo $sql;
							...
							if (($_SESSION['COMCODE']=="A") or ($_SESSION['COMCODE']=="B") or ($_SESSION['COMCODE']=="R")){	
								$sql="SELECT * FROM s_purmas where mid(REFNO, 1, 1)='".$_SESSION['COMCODE']."' and CANCEL!='1' order by id desc ";
							} else {
