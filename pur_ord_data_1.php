...
////////////////////////////////////////////// Database Connector /////////////////////////////////////////////////////////////
	...
	///////////////////////////////////// Registration /////////////////////////////////////////////////////////////////////////
		...
		if($_GET["Command"]=="new_inv")
		{
			...
			$sql="Select ORD_NO_1 from invpara";
			$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
			$row = mysqli_fetch_array($result);
			$tmpinvno="000000".$row["ORD_NO_1"];
			$lenth=strlen($tmpinvno);
			$invno=trim("ODN/ ").substr($tmpinvno, $lenth-8);
			$_SESSION["invno"]=$invno;
			
			$sql="Select ORD_NO_1 from tmpinvpara";
			$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
			$row = mysqli_fetch_array($result);
			$_SESSION["tmp_no_purord"]="ODN/".$row["ORD_NO_1"];
	...
	if($_GET["Command"]=="add_tmp")
		{
			...
			$ResponseXML .= "<salesdetails>";
                        ...
                        if($_SESSION["purType"] != "ST"){
                            $itemCode = $_SESSION["purType"].$_GET['autoCode'];
                        }else{
			...
			$sql="delete from tmp_purord_data where str_code='".$itemCode."' and tmp_no='".$_SESSION['tmp_no_purord']."' ";
			//$ResponseXML .= $sql;
			$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
...
//			} else {
				...
				('".$_SESSION["tmp_no_purord"]."', '".$_GET['invno']."', '".$itemCode."', '".$_GET['item']."', '".$_GET["partno"]."', ".$qty.", '".$_GET["cost"]."') ";
				$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 	
//			}
			...
			while($row = mysqli_fetch_array($result)){				
             	$ResponseXML .= "<tr>                              
							 ...
							 <td style=\"visibility: hidden\" >".number_format($row['rate'], 2, ".", ",")."</a></td>
	...
	if($_GET["Command"]=="setord")
	{
		...
		$salesord1=trim("ODN/ ").$_GET["salesrep"].trim(" / ").$need;
...
if($_GET["Command"]=="save_item")
{
	...
	$ResponseXML .= "<salesdetails>";
		...
		//	$_GET["CHECK_USER"] = false ;       //check user permission again
			...
			$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
			//echo $sql;
                        $refNo = $_SESSION["purType"] ."/". $_GET["invno"];
			if($row = mysqli_fetch_array($result)){
				$sql1="delete from s_ordmas_1 where REFNO='".$_GET["invno"]."'";
				$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 	
				
				$sql1="delete from s_ordtrn_1 where REFNO='".$_GET["invno"]."'";
				$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 	
			} else {
				$sql="Select ORD_NO_1 from invpara";
				$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
				...
				$tmpinvno="000000".$row["ORD_NO_1"];
				$lenth=strlen($tmpinvno);
                                $invno=trim("ODN/ ").substr($tmpinvno, $lenth-7);
				$_SESSION["invno"]=$invno;
				
				$sql1="update invpara set ORD_NO_1=ORD_NO_1+1";
				$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 
			}
			...
			$sql1="insert into s_ordmas_1(REFNO, SDATE, SUP_CODE, SUP_NAME, REMARK, DEP_CODE, DEP_NAME, REP_CODE, REP_NAME, cancel, S_date, LC_No, pi_no, Brand, tmp_no, vat_type, po_type) values ('".$refNo."', '".$_GET["invdate"]."', '".$_GET["supplier_code"]."','".$_GET["supplier_name"]."','".$_GET["remarks"]."','".$_GET["department"]."','".$dep_name."','".$_GET["salesrep"]."','".$rep_name."', '0','".$_GET["dte_shedule"]."','".$_GET["lc_no"]."', '".$_GET["pi_no"]."', '".$_GET["brand"]."',  '".$_SESSION['tmp_no_purord']."', '$vat', '".$_SESSION["purType"]."')";
			//echo $sql1;
			$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 	
			...
			$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
			while($row = mysqli_fetch_array($result)){
    			$sql1="insert into s_ordtrn_1(REFNO, SDATE, STK_NO, DESCRIPT, ORD_QTY, partno, CANCEL, tmp_no, RATE) values ('".$refNo."', '".$_GET["invdate"]."', '".$row["str_code"]."','".$row["str_description"]."','".$row["qty"]."','".$row["partno"]."','0', '".$_SESSION['tmp_no_purord']."','".$row["rate"]."')";
				//echo $sql1;
				$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 	
  			}
		//$_SESSION["print"]=1;			
...
{
	$sql1="update s_ordmas_1 set cancel='1' where REFNO='".$_GET["invno"]."'";
	$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 
	
	$sql1="update s_ordtrn_1 set CANCEL='1' where REFNO='".$_GET["invno"]."'";
	$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 
	echo "Canceled!";
}	
