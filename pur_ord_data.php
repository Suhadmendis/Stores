	...
	if($_GET["Command"]=="purord")
		{
			...
			if($row = mysqli_fetch_array($result)){
				...
				$ResponseXML .= "<REQNO><![CDATA[".$row['REQNO']."]]></REQNO>";
				...
				$ResponseXML .= "<Brand><![CDATA[".$row['Brand']."]]></Brand>";
                                ...
                                $ResponseXML .= "<entBy><![CDATA[".$row['user']."]]></entBy>";
                                $ResponseXML .= "<appBy><![CDATA[".$row['app_by']."]]></appBy>";
                                
                                $ResponseXML .= "<net_tot><![CDATA[".$row['net_tot']."]]></net_tot>";
                                $ResponseXML .= "<g_tot><![CDATA[".$row['g_tot']."]]></g_tot>";
...
//			$result =mysqli_query($GLOBALS['dbinv'],$sql) ; 
//			while($row = mysqli_fetch_array($result)){
//				$sql1="Insert into tmp_purord_data (tmp_no, str_invno, str_code, str_description, partno, qty, rate, subtotal)values 
//				('".$_SESSION['tmp_no_purord']."', '".$row['REFNO']."', '".$row['STK_NO']."', '".$row['DESCRIPT']."', '".$row["tax"]."', ".$row["ORD_QTY"].", ".$row["RATE"].", ".$row["sub_tot"].") ";
//			//$ResponseXML .= $sql;
//				$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 	
//			}
				...
				$ResponseXML .= "<sales_table><![CDATA[ <table><tr>
                              ...
                              <td width=\"100\"  background=\"\"><font color=\"#FFFFFF\">Cost</font></td>
                              <td width=\"100\"  background=\"\">Tax</td>
                              <td width=\"100\"  background=\"\"><font color=\"#FFFFFF\">SubtTotal</font></td>
...
//			$sql="Select * from tmp_purord_data where str_invno='".$_GET['invno']."'";
			...
			while($row = mysqli_fetch_array($result)){				
                            ...
                            $ResponseXML .= "<tr>                              
                             ...
                             <td >".number_format($row['ORD_QTY'], 0, ".", ",")."</a></td>
                             <td >".number_format($row['RATE'], 2, ".", ",")."</a></td>    
                             <td >".$option."</a></td>
                             <td >".number_format($row['sub_tot'], 2, ".", ",")."</a></td>    
                             </tr>";
	...
	$ResponseXML .= "<salesdetails>";
		...
		//	$_GET["CHECK_USER"] = false ;       //check user permission again
			...
			$rowPara = mysqli_fetch_array($resultPara);
                        ...
                        $refNo = $_GET["purType"]."/".$_GET["type"]."/".$_GET["invno"];
			
			$sql1="insert into s_ordmas(REFNO, SDATE, SUP_CODE, SUP_NAME, REMARK, DEP_CODE, DEP_NAME, REP_CODE, REP_NAME, cancel, S_date, LC_No, pi_no, Brand, tmp_no, REQNO, vat_type, type1, TYPE) values ('".$refNo."', '".$_GET["invdate"]."', '".$_GET["supplier_code"]."','".$_GET["supplier_name"]."','".$_GET["remarks"]."','".$_GET["department"]."','".$dep_name."','".$_GET["salesrep"]."','".$rep_name."', '2','".$_GET["dte_shedule"]."','".$_GET["lc_no"]."', '".$_GET["pi_no"]."', '".$_GET["brand"]."',  '".$_SESSION['tmp_no_purord']."', '".$_GET["reqno"]."', '$vat', '".$_GET["type"]."', '".$_GET["purType"]."')";
//			echo $sql1;
			$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 	
			...
			while($row = mysqli_fetch_array($result)){
//                                if($vat != "0"){
//                                    $afterNbt = $row["rate"] * (100 + $rowPara["nbt"])/100;
//                                    $afterVaT = $afterNbt * (100 + $rowPara["vatrate"])/100;
//                                    
//                                }else{
//                                    $afterVaT = $row["rate"];
//                                }
                                $sql1="insert into s_ordtrn(REFNO, SDATE, STK_NO, DESCRIPT, ORD_QTY, partno, CANCEL, tmp_no, RATE) values ('".$refNo."', '".$_GET["invdate"]."', '".$row["str_code"]."','".$row["str_description"]."','".$row["qty"]."','".$row["partno"]."','2', '".$_SESSION['tmp_no_purord']."','".$row["rate"]."')";
				//echo $sql1;
				$result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ; 	
  			}
                        
                        $sql1 = "update s_ordmas_1 set cancel='1' where REFNO='" . $_GET["reqno"] . "'";
                        $result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ;
                        $sql1 = "update s_ordtrn_1 set CANCEL='1' where REFNO='" . $_GET["reqno"] . "'";
                        $result1 =mysqli_query($GLOBALS['dbinv'],$sql1) ;
