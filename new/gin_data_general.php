...
function getno() {
    ...
    $sql = "select GINM from invpara";
    $result = $conn->query($sql);
    $row = $result->fetch();
    $tmpinvno = "000000" . $row["GINM"];
    $lenth = strlen($tmpinvno);
    return $invno = trim("IS/") . substr($tmpinvno, $lenth - 7);
}
...
if ($_GET["Command"] == "save_item") {
    try {
        ...
        if ($row = $result->fetch()) {
            ...
            $invno = getno();
            $sql = "update invpara set GINM=GINM+1";
            $conn->exec($sql);
        }
        ...
        foreach ($conn->query($sql) as $row) {
            ...
            $sql5 = "Insert into s_trn (STK_NO, SDATE, REFNO, QTY, LEDINDI, DEPARTMENT, DESCRIPt, cost) values('" . $row["str_code"] . "', '" . $_GET["invdate"] . "', '" . trim($invno) . "', " . $cur_qty . ", 'GUI','" . $_GET["from_dep"] . "', '" . $row["str_description"] . "', " . $rowItem["cost"] . ")";
            $conn->exec($sql5);
        }
        ...
        $sql1 = "insert into s_ginmas(SDATE, REF_NO, DEP_FROM, DEP_F_NAME, DEP_TO, DEP_T_NAME, tmp_no, JOB_NO, remark, TYPE, TYPE1) values ('" . $_GET["invdate"] . "', '" . $invno . "', '" . $_GET["from_dep"] . "', '" . $DESCRIPTION_from . "', '', '', '" . $_GET["tmpno"] . "', '" . $_GET["txt_jobno"] . "', '" . $_GET["txt_remarks"] . "', 'MING', '$type1')";
