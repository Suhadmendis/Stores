...
function getno() {
    ...
    $sql = "select GINA from invpara";
    $result = $conn->query($sql);
    $row = $result->fetch();
    $tmpinvno = "000000" . $row["GINA"];
    $lenth = strlen($tmpinvno);
    return $invno = trim("MRIS/") . substr($tmpinvno, $lenth - 7);
}
...
if ($_GET["Command"] == "save_item") {
    try {
        ...
        if ($row = $result->fetch()) {
            ...
            $invno = getno();
            $sql = "update invpara set GINA=GINA+1";
            $conn->exec($sql);
        }
        ...
        foreach ($conn->query($sql) as $row) {
            ...
            */
            ...
            $sql5 = "Insert into s_gintrn (STK_NO, SDATE, REFNO, QTY, LEDINDI, DEPARTMENT, DESCRIPt, cost) values('" . $row["str_code"] . "', '" . $_GET["invdate"] . "', '" . trim($invno) . "', " . $cur_qty . ", 'GINMI','" . $_GET["from_dep"] . "', '" . $row["str_description"] . "', " . $rowItem["cost"] . ")";
            $conn->exec($sql5);
            $sql5 = "Insert into s_gintrn (STK_NO, SDATE, REFNO, QTY, LEDINDI, DEPARTMENT, DESCRIPt, cost) values('" . $row["str_code"] . "', '" . $_GET["invdate"] . "', '" . trim($invno) . "', " . $cur_qty . ", 'GINMR','" . $_GET["to_dep"] . "', '" . $row["str_description"] . "', " . $rowItem["cost"] . ")";
            $conn->exec($sql5);
        }
        ...
        $sqlGlPost = "select * from gl_posting where docname = 'ISSUE NOTE GRL'";
        $ledgerRem = "Allocated Issue note";
        ...
        foreach ($conn->query($sqlGlPost) as $rowGlPost) {
            $sqlLedger = "Insert into ledger(l_refno, l_date, L_LMEM, l_code, l_amount, l_flag, l_flag1, acyear, ComCode, c_remarks) Values ('" . trim($invno) . "', '" . $ldate . "', '$ledgerRem', '" . $rowGlPost["l_code"] . "', " . $totCost . ", 'AISN', '" . $rowGlPost['entry_flag'] . "', '$ayear', '" . $_SESSION['company'] . "', '" . $_GET["txt_jobno"] . "')";
            $conn->exec($sqlLedger);
            if ($_GET["Command1"] == "getGl") {
                $msg .= "<tr>";
        ...
        $sql1 = "insert into s_ginmas(SDATE, REF_NO, DEP_FROM, DEP_F_NAME, DEP_TO, DEP_T_NAME, tmp_no, JOB_NO, remark, TYPE) values ('" . $_GET["invdate"] . "', '" . $invno . "', '" . $_GET["from_dep"] . "', '" . $DESCRIPTION_from . "', '" . $_GET["to_dep"] . "', '" . $DESCRIPTION_to . "', '" . $_GET["tmpno"] . "', '" . $_GET["txt_jobno"] . "', '" . $_GET["txt_remarks"] . "', 'ISA')";
    ...
    try {
        ...
        if ($row = $result->fetch()) {
            if ($row['cancel'] != "0") {
                ...
                /*
                $sql = "select * from s_gintrn where REFNO='" . $row['REF_NO'] . "'";
                foreach ($conn->query($sql) as $row1) {
                ...
                */
                ...
                $sql = "select allocated from s_gintrn where REFNO='" . $row['REF_NO'] . "'";
                $result = $conn->query($sql);
                $rowCheck = $result->fetch(); 
                
                if($rowCheck["allocated"]>0){
                    exit("Cannot Cancel! Allocation already took place!");
                }
                
                $sql = "update s_ginmas set cancel='1' where REF_NO = '" . $row['REF_NO'] . "'";
                $conn->exec($sql);

                $sql = "delete from s_gintrn where REFNO='" . $row['REF_NO'] . "'";
                $conn->exec($sql);
