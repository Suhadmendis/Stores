...
if ($_GET["Command"] == "save_item") {
    try {
        ...
        foreach ($conn->query($sql) as $row) {
            ...
            $sql1 = "update job_mas set trf_qty=trf_qty+" . $cur_qty . " where REF_NO='" . $_GET['txt_jobno'] . "'";
            $conn->exec($sql1);
...
//            $totCost += $cost;
            ...
            $sql = "Select COST from job_mas where REF_NO='" . $_GET['txt_jobno'] . "'";
            $result = $conn->query($sql);
            $rowJob = $result->fetch();

            $cost = $rowJob["COST"] * $cur_qty;
            $totCost += $cost;            
        ...
        foreach ($conn->query($sqlGlPost) as $rowGlPost) {
            $sqlLedger = "Insert into ledger(l_refno, l_date, L_LMEM, l_code, l_amount, l_flag, l_flag1, acyear, ComCode) Values ('" . trim($invno) . "', '" . $ldate . "', '$ledgerRem', '" . $rowGlPost["l_code"] . "', " . $totCost . ", 'FGR', '" . $rowGlPost['entry_flag'] . "', '$ayear', '" . $_SESSION['company'] . "')";
            $conn->exec($sqlLedger);
