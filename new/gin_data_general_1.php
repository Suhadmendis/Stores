...
function getno() {
    ...
    $sql = "select GIND from invpara";
    $result = $conn->query($sql);
    $row = $result->fetch();
    $tmpinvno = "000000" . $row["GIND"];
    $lenth = strlen($tmpinvno);
    return $invno = trim("DSR/") . substr($tmpinvno, $lenth - 7);
}
...
//         str_code, str_description, cur_qty, str_invno
        $sql = "Insert into tmp_stock_adjust_data (str_code, str_description, cur_qty, str_invno, val)values 
			('" . $_GET['itemCode'] . "', '" . $_GET['itemDesc'] . "', " . $qty . ",'" . $_GET['tmpno'] . "', $rate) ";
//        echo $sql;
        $result = $conn->query($sql);
    }
...
if ($_GET["Command"] == "save_item") {
    try {
        ...
        if ($row = $result->fetch()) {
            ...
            $invno = getno();
            $sql = "update invpara set GIND=GIND+1";
            $conn->exec($sql);
        }
...
//            $rowItem = $conn->query($sql)->fetch();
            ...
            $sql5 = "Insert into s_trn (STK_NO, SDATE, REFNO, QTY, LEDINDI, DEPARTMENT, DESCRIPt, cost) values('" . $row["str_code"] . "', '" . $_GET["invdate"] . "', '" . trim($invno) . "', " . $cur_qty . ", 'DSR','" . $_GET["from_dep"] . "', '" . $row["str_description"] . "', " . $row["val"] . ")";
            $conn->exec($sql5);
        }
        /*
        ...
        $type1 = "normal";
        $sqlGlPost = "select * from gl_posting where docname = 'General Issue'"; 
        if ($_GET["type1"] == "direct") {
            $type1 = "direct";
            $sqlGlPost .= " and action != 'ADD_EXP'";
        }else{
            $sqlGlPost .= " and action != 'ADD_EXP_SLS'";
        }

        $ledgerRem = "General Issue";
        ...
        foreach ($conn->query($sqlGlPost) as $rowGlPost) {
            $sqlLedger = "Insert into ledger(l_refno, l_date, L_LMEM, l_code, l_amount, l_flag, l_flag1, acyear, ComCode) Values ('" . trim($invno) . "', '" . $ldate . "', '$ledgerRem', '" . $rowGlPost["l_code"] . "', " . $totCost . ", 'GISN', '" . $rowGlPost['entry_flag'] . "', '$ayear', '" . $_SESSION['company'] . "')";
            $conn->exec($sqlLedger);
        ...
        */
         ...
         */
        $sql = "select ADD1 from vendor where CODE = '".$_GET["cusText"]."'";
        $row = $conn -> query($sql) -> fetch();
        
        $sql1 = "insert into s_ginmas(SDATE, REF_NO, DEP_FROM, DEP_F_NAME, DEP_TO, DEP_T_NAME, tmp_no, JOB_NO, remark, TYPE, C_CODE, CUS_NAME, C_ADD1) values ('" . $_GET["invdate"] . "', '" . $invno . "', '" . $_GET["from_dep"] . "', '" . $DESCRIPTION_from . "', '', '', '" . $_GET["tmpno"] . "', '" . $_GET["txt_jobno"] . "', '" . $_GET["txt_remarks"] . "', 'DSR', '" . $_GET["cusText"] . "', '" . $_GET["cusTextName"] . "', '".$row["ADD1"]."')";
        $conn->exec($sql1);
