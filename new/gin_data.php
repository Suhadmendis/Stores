...
if ($_GET["Command"] == "save_item") {
    try {
        ...
        $sql1 = "insert into s_ginmas(SDATE, REF_NO, DEP_FROM, DEP_F_NAME, DEP_TO, DEP_T_NAME, tmp_no, JOB_NO, remark, TYPE) values ('" . $_GET["invdate"] . "', '" . $invno . "', '" . $_GET["from_dep"] . "', '" . $DESCRIPTION_from . "', '" . $_GET["to_dep"] . "', '" . $DESCRIPTION_to . "', '" . $_GET["tmpno"] . "', '" . $_GET["txt_jobno"] . "', '" . $_GET["txt_remarks"] . "', 'MIN')";
    ...
    if (($stname == "") || ($stname == "isa") || ($stname == "fg")) {
        ...
        if ($row = $result->fetch()) {
            ...
            if (($stname == "")) {
                ...
                $sql = "Select * from job_mas where REF_NO='" . $row["JOB_NO"] . "'";
                $result = $conn->query($sql);
                $rowJob = $result->fetch();
                $ResponseXML .= "<jobQty><![CDATA[" . $rowJob["qty"] . "]]></jobQty>";
            }
...
//                    $ResponseXML .= "<test><![CDATA[" . $sql . "]]></test>";
        ...
        $sql = "Select JOB_NO from s_ginmas where REF_NO='" . $row['JOB_NO'] . "'";
        $result = $conn->query($sql);
        $row = $result->fetch();

        $sql = "Select QTY from s_mrntrn where REFNO='" . $_GET['refno'] . "'";
        $result = $conn->query($sql);
        $rowTrn = $result->fetch();
