...
//            $sql = "select jobCost,COST,rawWaste from s_mas where STK_NO = '" . $row["stk_no"] . "'";
		...
		('" . $_GET['invdate'] . "','" . $row["stk_no"] . "','" . trim($invno) . "', " . $row["rate"] . ",'INV', '" . $_GET['department'] . "', '" . $rowItem["COST"] . "','" . $row["descript"] . "')";
            $result = $conn->query($sql);
	...
	('" . $invno . "', '" . $_GET['invdate'] . "','INV' ,'" . $_GET["customercode"] . "', '" . $_GET["customername"] . "','" . $_GET["cont_p"] . "','" . $mtot1 . "','" . $svatValue . "', '$vatType', '" . $_GET["tmpno"] . "','" . $_GET['txt_remarks'] . "','" . $nbt . "','" . $mgrand_tot . "','" . $_GET['salesrep'] . "','" . $row['vatrate'] . "', '" . $row['nbt'] . "', '" . $_SESSION['UserName'] . "','" . $_GET['department'] . "','" . $_GET['DANO'] . "','" . $_GET['txt_minno'] . "')";
//        echo $sql;
        $result = $conn->query($sql);
...
//        $ResponseXML .= "<tmp_no><![CDATA[" . $row["tmp_no"] . "]]></tmp_no>";
        ...
        $ResponseXML .= "<msg><![CDATA[" . $msg . "]]></msg>";
        */
//        $sql = "delete from tmp_po_data where tmp_no='" . $_SESSION["tmp_dinv"] . "'";
//        $result = $conn->query($sql);


        $sql = "Select count(*) as num from s_trn where REFNO='" . $_GET['refno'] . "'";
        $row = $conn->query($sql)->fetch();
        //qty and rate swap!
//        foreach ($conn->query($sql) as $row1) {
//            $subtotal = $row1['QTY'] * $row1['cost'];
//            $sql = "Insert into tmp_po_data (stk_no, descript, qty, rate,subtot, tmp_no) values 
//			('" . $row1['STK_NO'] . "', '" . $row1['DESCRIPt'] . "', " . $row1['cost'] . ", " . $row1['QTY'] . ",'" . $subtotal . "','" . $_SESSION["tmp_dinv"] . "') ";
//            $result_t = $conn->query($sql);
//        }
//
//
...
//        $i = 1;
//        $mtot = 0;
//        $sql = "Select * from tmp_po_data where tmp_no='" . $_SESSION["tmp_dinv"] . "'";
//        foreach ($conn->query($sql) as $row1) {
//
//            $ResponseXML .= "<tr>                              
//                             <td>" . $row1['stk_no'] . "</td>
...
//        
//        $ResponseXML .= "<vatType><![CDATA[" . $row["VAT"] . "]]></vatType>";
        $ResponseXML .= "<item_count><![CDATA[" . ($row["num"]+1) . "]]></item_count>";
//        
//        $ResponseXML .= "<nbt><![CDATA[" . number_format($row['BTT'], 2, ".", ",") . "]]></nbt>";
//        $ResponseXML .= "<gtot><![CDATA[" . number_format($row['GRAND_TOT'], 2, ".", ",") . "]]></gtot>";
//        $ResponseXML .= "<subtot><![CDATA[" . number_format($mtot, 2, ".", ",") . "]]></subtot>";
...
if ($_GET["Command"] == "update_list_dsr") {
    $ResponseXML = "";
    $ResponseXML .= "<table class=\"table\">
	            <tr>
                        <th width=\"121\">Reference No</th>
