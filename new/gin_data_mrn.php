...
function getno() {
    ...
    $sql = "select MRN from invpara";
    $result = $conn->query($sql);
    $row = $result->fetch();
    $tmpinvno = "000000" . $row["MRN"];
    $lenth = strlen($tmpinvno);
    return $invno = trim("MRN/") . substr($tmpinvno, $lenth - 7);
}
...
//        exit ($sql);
        if ($row = $result->fetch()) {
            ...
            $invno = getno();
            $sql = "update invpara set MRN=MRN+1";
            $conn->exec($sql);
        }
        ...
        foreach ($conn->query($sql) as $row) {
            ...
            $sql5 = "Insert into s_mrntrn (STK_NO, SDATE, REFNO, QTY, LEDINDI, DEPARTMENT, DESCRIPt, TYPE) values('" . $row["str_code"] . "', '" . $_GET["invdate"] . "', '" . trim($invno) . "', " . $cur_qty . ", 'GINI','" . $_GET["from_dep"] . "', '" . $row["str_description"] . "', 'FG')";
            $conn->exec($sql5);
            $sql5 = "Insert into s_mrntrn (STK_NO, SDATE, REFNO, QTY, LEDINDI, DEPARTMENT, DESCRIPt, TYPE) values('" . $row["str_code"] . "', '" . $_GET["invdate"] . "', '" . trim($invno) . "', " . $cur_qty . ", 'GINR','" . $_GET["to_dep"] . "', '" . $row["str_description"] . "', 'FG')";
            $conn->exec($sql5);
        }
        ...
        $sql1 = "insert into s_mrnmas(SDATE, REF_NO, DEP_FROM, DEP_F_NAME, DEP_TO, DEP_T_NAME, tmp_no, JOB_NO, remark, TYPE) values ('" . $_GET["invdate"] . "', '" . $invno . "', '" . $_GET["from_dep"] . "', '" . $DESCRIPTION_from . "', '" . $_GET["to_dep"] . "', '" . $DESCRIPTION_to . "', '" . $_GET["tmpno"] . "', '" . $_GET["txt_jobno"] . "', '" . $_GET["txt_remarks"] . "', 'MRN')";
