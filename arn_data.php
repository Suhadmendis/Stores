...
//    $sql = "Select ARN_TMP from invpara where COMCODE='" . $_SESSION['company'] . "'";
    $sql = "Select ARN_TMP from invpara";
    $result = $db->RunQuery($sql);
    $row = mysql_fetch_array($result);
    $tmpinvno = "000000" . $row["ARN_TMP"];
    $lenth = strlen($tmpinvno);
    $invno =  $_SESSION['company'] . trim("GT") . substr($tmpinvno, $lenth - 8);
...
if ($_GET["Command"] == "arn") {
    ...
    if ($row = mysql_fetch_array($result)) {
        ...
        $ResponseXML .= "<type><![CDATA[".$row['type1']."]]></type>";
        $ResponseXML .= "<purType><![CDATA[".$row['TYPE']."]]></purType>";
    }
    ...
    $ResponseXML .= "<sales_table><![CDATA[ <table><tr>
                              ...
                              <td width=\"300\"  background=\"\"><font color=\"#FFFFFF\">Pending Qty</font></td>
...
//		$descr= trim($row_selling['PART_NO'])  . " " . trim($row_selling['DESCRIPT'])  . " " . trim($row_selling['SUBSTITUTE']) .  " " . trim($row_selling['jobno']);
		$descr= trim($row_selling['PART_NO'])  . " " . trim($row["DESCRIPT"]);
        $ResponseXML .= "<tr>                              
                             <td ><input type=\"text\" size=\"15\" name=" . $itemcode . " id=" . $itemcode . "   value=" . $row['STK_NO'] . " class=\"text_purchase3\" disabled=\"disabled\"/></td>
							  ...
							  <td ><input type=\"text\" size=\"15\" name=" . $ord_qty_org . " id=" . $ord_qty_org . "  value=" . ($row['ORD_QTY']) . " class=\"text_purchase3\" disabled=\"disabled\"/></td>
							  <td ><input type=\"text\" size=\"15\" name=" . $ord_qty . " id=" . $ord_qty . "  value=" . ($row['ORD_QTY']-$row['REC_QTY']) . " class=\"text_purchase3\" disabled=\"disabled\"/></td>
							
							 <td ><input type=\"hidden\" size=\"15\" name=" . $fob . " id=" . $fob . "  class=\"text_purchase3\"/></td>
							 <td ><input type=\"text\" size=\"15\" name=" . $qty . " id=" . $qty . "  class=\"text_purchase3\" onkeyup=\"cal_subtot('" . $i . "', '" . $mcou . "');\"/></td>
							 ...
							 <td ><input type=\"hidden\" size=\"15\" name=" . $cost . " id=" . $cost . " value=" . $row['RATE'] . " class=\"text_purchase3\" onkeyup=\"cal_subtot('" . $i . "', '" . $mcou . "');\" disabled=\"disabled\"/></td>
							 ...
							 <td ><input type=\"hidden\" size=\"15\" name=" . $subtotal . " id=" . $subtotal . "  class=\"text_purchase3\" disabled=\"disabled\"/></td>
							</tr>";
        $i = $i + 1;
    }
...
//        $sql_invpara = "SELECT * from invpara where COMCODE='" . $_SESSION['company'] . "'";
        ...
        $sql = "select * from s_purmas_tmp where REFNO='" . $_POST["invno"] . "'";
        ...
        if ($row = mysql_fetch_array($result)) {
            ...
            /*
            $sql1 = "insert into c_bal(REFNO, SDATE, CUSCODE, AMOUNT, BALANCE, DEP, SAL_EX, Cancell, brand, DEV, trn_type, vatrate, old, flag1, active, totpay, company) values ('" . $_POST["invno"] . "', '" . $_POST["invdate"] . "', '" . $_POST["supplier_code"] . "', '" . $_POST["total_value"] . "', '" . $_POST["total_value"] . "', '" . $_POST["department"] . "', '" . $_POST["salesrep"] . "', '0', '" . $_POST["brand"] . "', '" . $_SESSION['dev'] . "', 'ARN', '" . $vatrate . "', '0', 0, 1, 0, '".$_SESSION['company']."')";
            //echo $sql1;
	  ...
	  ('" . $_POST["invno"] . "', '" . $_POST["invdate"] . "', '" . $_POST["invno"] . "', " . $_POST["total_value"] . ", " . $_POST["total_value"] . ", " . (-1 * $_POST["total_value"]) . ", '" . $_POST["supplier_code"] . "', '" . $supplier_name . "', '" . $_SESSION['dev'] . "', 0, 0, 0, '0', 'O', 'ARN', 'OVER')";
        ...
        */
            ...
            $sql = "select TYPE, type1 from s_ordmas where REFNO = '".$_POST["orderno1"]."'";
            $result=mysql_query($sql, $dbinv);
            $rowOrd= mysql_fetch_array($result);
            $refNo = $rowOrd["TYPE"]."/".$rowOrd["type1"]."/".$_POST["invno"];
            
            $sql1 = "insert into s_purmas_tmp(REFNO, SDATE, ORDNO, LCNO, pi_no, COUNTRY, SUP_CODE, SUP_NAME, REMARK, DEPARTMENT, AMOUNT, PUR_DATE,
TYPE, type1, brand) values ('" . $refNo . "', '" . $_POST["invdate"] . "', '" . $_POST["orderno1"] . "', '" . $_POST["lc_no"] . "', '" . $_POST["pi_no"] . "', '" . $_POST["country"] . "', '" . $_POST["supplier_code"] . "', '" . $supplier_name . "', '" . $_POST["textarea"] . "', '" . $_POST["department"] . "', '" . $_POST["total_value"] . "', '" . $_POST["dte_dor"] . "', '" . $rowOrd["type1"] . "', '" . $rowOrd["TYPE"] . "', '" . $_POST["brand"] . "')";
            //echo $sql1;
            $result1=mysql_query($sql1, $dbinv);
        ...
        if($_POST["purtype"]=="Import"){
           $sqlGlPost = "select * from gl_posting where docname = 'ARRIVAL' and action = 'ADD_PURCHASE_I'"; 
        }else{
           $sqlGlPost = "select * from gl_posting where docname = 'ARRIVAL' and action = 'ADD_PURCHASE_L'"; 
        }
        
        $result=mysql_query($sqlGlPost, $dbinv);
        $rowGlPost = mysql_fetch_array($result);

        $sqlLedger = "Insert into ledger(l_refno, l_date, l_code, l_amount, l_flag, l_flag1, acyear, ComCode) Values ('" . $_POST["invno"] . "', '" . $_POST["invdate"] . "', '" . $rowGlPost["l_code"] . "', " . $orgAmount . ", 'ARN', '" . $rowGlPost["entry_flag"] . "', '$ayear', '" . $_SESSION['company'] . "')";
        ...
        $sqlLedger = "Insert into ledger(l_refno, l_date, l_code, l_amount, l_flag, l_flag1, acyear, ComCode) Values ('" . $_POST["invno"] . "', '" . $_POST["invdate"] . "', '" . $rowGlPost["l_code"] . "', " . $amount . ", 'ARN', '" . $rowGlPost["entry_flag"] . "', '$ayear', '" . $_SESSION['company'] . "')";
        $result1=mysql_query($sqlLedger, $dbinv);
        if ($result1==false){ $sql_status=$sqlLedger; }
        ...
        $sqlLedger = "Insert into ledger(l_refno, l_date, l_code, l_amount, l_flag, l_flag1, acyear, ComCode) Values ('" . $_POST["invno"] . "', '" . $_POST["invdate"] . "', '" . $rowGlPost["l_code"] . "', " . $vatAmount . ", 'ARN', '" . $rowGlPost['entry_flag'] . "', '$ayear', '" . $_SESSION['company'] . "')";
        $result1=mysql_query($sqlLedger, $dbinv);
        ...
        */
        ...
        while ($i < $_POST["count"]) {
            ...
            $ord_qty_org_name = "ord_qty_org" . $i;
            $fob_name = "fob" . $i;
            $qty_name = "qty" . $i;
...
//                }
//                $sql4 = "insert into s_purtrn_tmp(REFNO, SDATE, STK_NO, DESCRIPT, COST, MARGIN, SELLING, REC_QTY, FOB, DEPARTMENT, QTYINHAND, O_QTY, 
...
//                $sql4 = "insert into s_purtrn_tmp(REFNO, SDATE, STK_NO, DESCRIPT, MARGIN, SELLING, REC_QTY, COST) values ('" . $refNo . "', '" . $_POST["invdate"] . "', '" . $_POST[$itemcode_name] . "', '" . $_POST[$itemname_name] . "', " . $margin_name . ", " . $_POST[$selling_name] . ", " . $_POST[$qty_name] . ", $cost)";
                $sql4 = "insert into s_purtrn_tmp(REFNO, SDATE, STK_NO, DESCRIPT, MARGIN,  REC_QTY, COST, O_QTY) values ('" . $refNo . "', '" . $_POST["invdate"] . "', '" . $_POST[$itemcode_name] . "', '" . $_POST[$itemname_name] . "', " . $margin_name . ", " . $_POST[$qty_name] . ", $cost, " . $_POST[$ord_qty_org_name] . ")";
			   	$result1=mysql_query($sql4, $dbinv);
				if ($result1==false){ $sql_status='41 ' . $sql4; }
                $sql1 = "update s_ordtrn set REC_QTY=REC_QTY +'". $_POST[$qty_name] . "' where  STK_NO='" . $_POST[$itemcode_name] . "' and REFNO='" . $_POST["orderno1"] . "'";
//		echo $sql1;
                                $result1=mysql_query($sql1, $dbinv);
...
//                }
             ...
             //   }
                ...
                $sqlPre = "select QTYINHAND from s_mas where STK_NO='" . $_POST[$itemcode_name] . "'";
                $result=mysql_query($sqlPre, $dbinv);
                $rowPre = mysql_fetch_array($result);
                if($rowPre["QTYINHAND"]<0){
                    $sql1 = "insert into s_trn(STK_NO, SDATE, QTY, LEDINDI, REFNO, DEPARTMENT, seri_no, Dev, SAL_EX, ACTIVE, DONO) values ('" . $_POST[$itemcode_name] . "', '" . $_POST["invdate"] . "', '0', 'TRN', 'STK UPDATE', '" . $_POST["department"] . "', '', '" . $_SESSION['dev'] . "', '', '1', '')";
                    $result1=mysql_query($sql1, $dbinv);
                    if ($result1==false){ $sql_status=51; }
                    
                    $sql1 = "update s_mas set COST=" . $cost . ", acc_cost=" . $m_totval . ", SELLING='" . $_POST[$selling_name] . "', AR_selling='" . $_POST[$selling_name] . "', MARGIN ='" . $margin . "', QTYINHAND=" . $_POST[$qty_name] . " where  STK_NO='" . $_POST[$itemcode_name] . "'";
                    $result1=mysql_query($sql1, $dbinv);
               ...
               if ($row3 = mysql_fetch_array($result3)) {
                    ...
                    if($rowPre["QTYINHAND"]<0){
                        $sql1 = "update s_submas set QTYINHAND=" . $_POST[$qty_name] . " where STK_NO='" . $_POST[$itemcode_name] . "' and STO_CODE='" . $_POST["department"] . "'";
                        //	echo $sql1;
                        $result1=mysql_query($sql1, $dbinv);
...
*/
//                $sql1 = "update s_ordmas set cancel='2' where REFNO='" . $_POST["orderno1"] . "'";
//				$result1=mysql_query($sql1, $dbinv);
//				if ($result1==false){ $sql_status=9; }
//               
//
//                $sql1 = "update s_ordtrn set CANCEL='2' where REFNO='" . $_POST["orderno1"] . "'";
//				$result1=mysql_query($sql1, $dbinv);
...
/*
        ...
        $sql = "select * from s_ordtrn where REC_QTY != ORD_QTY and REFNO='" . $_POST["orderno1"] . "'";
        $result=mysql_query($sql, $dbinv);
        
        if(mysql_num_rows($result) == 0){
            $sql = "update s_ordmas set cancel='1' where REFNO='" . $_POST["orderno1"] . "'";
            $result=mysql_query($sql, $dbinv);
            if ($result==false){ $sql_status=9; }
            
            $sql = "update s_ordtrn set CANCEL='1' where REFNO='" . $_POST["orderno1"] . "'";
            $result=mysql_query($sql, $dbinv);
            if ($result==false){ $sql_status=9; }
        }
        
		if ($sql_status==0){
//        	$sql1 = "update invpara set ARN_TMP=ARN_TMP+1 where COMCODE='" . $_SESSION['company'] . "'";
        	$sql1 = "update invpara set ARN_TMP=ARN_TMP+1";
			$result1=mysql_query($sql1, $dbinv);
       		mysql_query("COMMIT", $dbinv);
       	 	echo "Saved";
		} else {
			mysql_query("ROLLBACK", $dbinv);
			echo "Error no $sql_status";
		}	
    } else {
        echo "no";
    }
...
if ($_GET["Command"] == "pass_arnno") {
    $ResponseXML = "";
    $ResponseXML .= "<salesdetails>";
    $sql = "Select * from s_purmas_tmp where REFNO='" . $_GET['arnno'] . "'";
    $result = $db->RunQuery($sql);
    if ($row = mysql_fetch_array($result)) {
        $ResponseXML .= "<REFNO><![CDATA[" . $row["REFNO"] . "]]></REFNO>";
        $ResponseXML .= "<SDATE><![CDATA[" . $row["SDATE"] . "]]></SDATE>";
        ...
        $mcou = 0;
        $sql = "Select count(*) as mcou from s_purtrn_tmp where REFNO='" . $_GET['arnno'] . "'";
        $result = $db->RunQuery($sql);
        $row = mysql_fetch_array($result);
        $mcou = $row["mcou"] + 1;
        ...
        $i = 1;
        $tot = 0;
        $sql = "Select * from s_purtrn_tmp where REFNO='" . $_GET['arnno'] . "'";
        $result = $db->RunQuery($sql);
        while ($row = mysql_fetch_array($result)) {
...
if ($_GET["Command"] == "cancel_inv") {
    ...
    $sql = "select CANCEL from s_purmas_tmp where REFNO='".$_GET["invno"]."' order by id desc";
    $result = $db->RunQuery($sql);
    $row = mysql_fetch_array($result);
    //echo $sql;
    if ($row["CANCEL"]=="1"){
        ...
        die ("Cannot Cancelled! Already entered by Accounting Department! ");
    }
    
    $sql1 = "update s_purmas_tmp set CANCEL='1' where REFNO='" . $_GET["invno"] . "'";
    $result1 = $db->RunQuery($sql1);

    $sql1 = "update s_purtrn_tmp set CANCEL='1' where REFNO='" . $_GET["invno"] . "'";
    $result1 = $db->RunQuery($sql1);
    ...
    $sql1 = "select * from s_purtrn_tmp where REFNO='" . $_GET["invno"] . "'";

    $result1 = $db->RunQuery($sql1);
    while ($row1 = mysql_fetch_array($result1)) {
        $sql2 = "update s_ordtrn set REC_QTY=REC_QTY-" . $row1["REC_QTY"] . " where STK_NO='" . $row1["STK_NO"] . "' and REFNO = '".$_GET["orderno1"]."'";
