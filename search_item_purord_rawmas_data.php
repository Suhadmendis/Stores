...
//								$sql = mysql_query("SELECT * from s_rawmas where STK_NO like  '$letters%' and BRAND_NAME='".$_SESSION["brand"]."' order by STK_NO limit 50") or die(mysql_error());
//								} else {
        $sql = mysql_query("SELECT * from s_rawmas where STK_NO like  '$letters%' order by STK_NO limit 50") or die(mysql_error());
        //}
    } else if ($_GET["mstatus"] == "itemname") {
        $letters = $_GET['itemname'];
        $letters = preg_replace("/[^a-z0-9 ]/si", "", $letters);
//								if (isset($_SESSION["brand"])==true){
//									$sql = mysql_query("SELECT * from s_rawmas where DESCRIPT like  '$letters%' and BRAND_NAME='".$_SESSION["brand"]."' order by STK_NO limit 50") or die(mysql_error()) or die(mysql_error());
//								} else {
        $sql = mysql_query("SELECT * from s_rawmas where DESCRIPT like  '$letters%' order by STK_NO limit 50") or die(mysql_error()) or die(mysql_error());
//								}
    } else {
...
if ($_GET["Command"] == "search_inv") {
    ...
    if ($_GET["mstatus"] == "invno") {
        ...
        //echo $a;
		if (($_GET["stname"]!="arn")&&($_GET["stname"]!="arn_1")){
                        $sql = mysql_query("SELECT * from s_ordmas where REFNO like  '$letters%' order by SDATE desc  limit 50") or die(mysql_error());
                } else if(($_GET["stname"]=="arn_1")){
                        $sql = mysql_query("SELECT * from s_purmas_tmp where CANCEL='0' and REFNO like  '$letters%' order by SDATE desc  limit 50") or die(mysql_error());
                } else {
			$sql = mysql_query("SELECT * from s_ordmas where cancel='0' and REFNO like  '$letters%' order by SDATE desc  limit 50") or die(mysql_error());
		}	
    } else if ($_GET["mstatus"] == "customername") {
        $letters = $_GET['customername'];
        //$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
		if (($_GET["stname"]!="arn")&&($_GET["stname"]!="arn_1")){
        	$sql = mysql_query("SELECT * from s_ordmas where SUP_NAME like  '$letters%' order by SDATE desc  limit 50") or die(mysql_error()) or die(mysql_error());
		} else if(($_GET["stname"]=="arn_1")){
                        $sql = mysql_query("SELECT * from s_purmas_tmp where CANCEL='0' and SUP_NAME like  '$letters%' order by SDATE desc  limit 50") or die(mysql_error());
                } else {
			$sql = mysql_query("SELECT * from s_ordmas where cancel='0' and SUP_NAME like  '$letters%' order by SDATE desc  limit 50") or die(mysql_error()) or die(mysql_error());
		}	
    } else {
        $letters = $_GET['customername'];
        //$letters = preg_replace("/[^a-z0-9 ]/si","",$letters);
